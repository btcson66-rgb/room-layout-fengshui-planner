...
// --- 合併與轉址 ---------------------------------------------------------
// 合併重複主題只有在「舊網址真的 301 到新頁」時才會轉移權重。這裡把三件
// 事情一起守住，任何一項破了都是靜默失效，不會有人在上線後發現：
//   1. dist/_redirects 有被產出，而且行數與 redirects.mjs 一致。
//   2. 轉址來源不能還在 sitemap 裡（Google 會抓到一批 301 的 URL）。
//   3. 轉址來源不能還 build 出 HTML（Cloudflare Pages 會優先回傳檔案）。
//   4. 轉址目標必須真的存在於 dist。
let redirectsFile = '';
try {
  redirectsFile = await fs.readFile(path.join(distRoot, '_redirects'), 'utf8');
} catch {
  redirectsFile = '';
}
check('redirects:file-exists', redirectsFile.length > 0, 'dist/_redirects must be generated by postbuild');
const redirectRuleLines = redirectsFile.split('\n').filter((line) => line.trim() !== '' && !line.startsWith('#'));
check('redirects:rule-count', redirectRuleLines.length === redirects.length, `${redirectRuleLines.length} vs ${redirects.length}`);
for (const entry of redirects) {
  check(
    `redirect-not-in-sitemap:${entry.from}`,
    !sitemapUrls.includes(`https://roomfeng.win${entry.from}`),
    'a 301 source must never stay in the sitemap',
  );
  check(`redirect-source-gone:${entry.from}`, !distRoutes.has(entry.from), 'source still builds a page; the 301 would never fire');
  check(`redirect-target-exists:${entry.to}`, distRoutes.has(entry.to), 'redirect target is missing from dist');
  check(`redirect-not-home:${entry.from}`, entry.from.startsWith('/zh/blog/') ? entry.to !== '/' : true, 'merged articles must not all land on the home page');
}

// --- 工具轉換 -----------------------------------------------------------
// RoomFeng 的差異化是「讀者可以真的規劃房間」。每一篇公開文章都必須帶著
// 一個具體的 Room Planner 入口，否則內容跟一般風水部落格沒有分別。
const articlesWithoutPlannerCta = [];
for (const slug of reviewReadyBlogSlugs) {
  const file = path.join(distRoot, 'zh', 'blog', slug, 'index.html');
  let html = '';
  try {
    html = await fs.readFile(file, 'utf8');
  } catch {
    check(`planner-cta-output:${slug}`, false, file);
    continue;
  }
  if (!/data-planner-cta/.test(html)) articlesWithoutPlannerCta.push(slug);
}
check('planner-cta-on-every-article', articlesWithoutPlannerCta.length === 0, articlesWithoutPlannerCta.slice(0, 5));

// --- 內鏈契約 -----------------------------------------------------------
// 每篇公開文章至少要有：1 個主題 Hub、2–5 篇高度相關文章、1 個工具入口。
// 上限同樣重要——塞 10–30 條無關連結會稀釋每一條的價值。
const linkContractFailures = [];
for (const slug of reviewReadyBlogSlugs) {
  const html = await fs.readFile(path.join(distRoot, 'zh', 'blog', slug, 'index.html'), 'utf8');
  const relatedBlock = html.match(/<nav class="related-links"[\s\S]*?<\/nav>/)?.[0] ?? '';
  const hrefs = [...relatedBlock.matchAll(/href="([^"]+)"/g)].map((match) => match[1]);
  const hubLinks = hrefs.filter((href) => href.startsWith('/zh/category/'));
  const articleLinks = hrefs.filter((href) => href.startsWith('/zh/blog/') && href !== `/zh/blog/${slug}/`);
  const toolLinkCount = hrefs.filter((href) => /^\/(zh|en)\/[a-z-]+\/$/.test(href) && !href.startsWith('/zh/blog/') && !href.startsWith('/zh/category/')).length;
  if (hubLinks.length !== 1) linkContractFailures.push(`${slug}: hub links = ${hubLinks.length}`);
  else if (articleLinks.length < 2 || articleLinks.length > 5) linkContractFailures.push(`${slug}: related articles = ${articleLinks.length}`);
  else if (toolLinkCount < 1) linkContractFailures.push(`${slug}: tool links = ${toolLinkCount}`);
}
check('internal-link-contract', linkContractFailures.length === 0, linkContractFailures.slice(0, 8));

// --- 文章頁重量 ---------------------------------------------------------
// affiliate 輪替池是 inline JSON，會直接算進 HTML 文件大小，也就是 LCP 之前
// 必須先下載的位元組。整份 194 筆目錄曾經佔掉每篇文章 182 KB 中的 146 KB。
// /support/ 是目錄頁，例外；其他頁面的池子必須維持有上限。
const heavyArticlePages = [];
for (const slug of reviewReadyBlogSlugs) {
  const html = await fs.readFile(path.join(distRoot, 'zh', 'blog', slug, 'index.html'), 'utf8');
  const poolMatch = html.match(/data-affiliate-products[^>]*>(\[.*?\])<\/script>/s);
  let poolCount = 0;
  try { poolCount = poolMatch ? JSON.parse(poolMatch[1]).length : 0; } catch { poolCount = 0; }
  if (poolCount > 24 || html.length > 120 * 1024) heavyArticlePages.push(`${slug}: pool=${poolCount} bytes=${html.length}`);
}
check('article-page-weight', heavyArticlePages.length === 0, heavyArticlePages.slice(0, 5));

// --- Hub 頁 -------------------------------------------------------------
// FAQPage schema 只有在畫面真的顯示 FAQ 時才可以輸出。分類頁第 1 頁是 Hub
// （有 FAQ 區塊），第 2 頁之後是純清單（不得帶 FAQPage）。
for (const category of reviewReadyCategorySlugs) {
  const hubHtml = await fs.readFile(path.join(distRoot, 'zh', 'category', category, 'index.html'), 'utf8');
  const showsFaq = /id="hub-faq"/.test(hubHtml);
  const emitsFaqSchema = /"@type":"FAQPage"/.test(hubHtml);
  check(`hub:${category}:faq-schema-matches-page`, showsFaq === emitsFaqSchema, `visible=${showsFaq} schema=${emitsFaqSchema}`);
  check(`hub:${category}:has-lead`, /id="core-questions"/.test(hubHtml), 'hub page needs a real intro, not just a post list');
  check(`hub:${category}:has-breadcrumb`, /aria-label="Breadcrumb"/.test(hubHtml), 'breadcrumb required');
  check(`hub:${category}:collection-schema`, /"@type":"CollectionPage"/.test(hubHtml), 'CollectionPage schema required');
  const paginated = path.join(distRoot, 'zh', 'category', category, '2', 'index.html');
  let page2 = '';
  try {
    page2 = await fs.readFile(paginated, 'utf8');
  } catch {
    page2 = '';
  }
  if (page2) {
    check(`hub:${category}:page2-no-faq-schema`, !/"@type":"FAQPage"/.test(page2), 'paginated pages must not repeat the hub FAQ schema');
  }
}

// --- FAQ schema 與畫面一致 ----------------------------------------------
// 「參考資料」曾經被當成一個 FAQ Question 輸出到 283 篇文章的 FAQPage 裡。
const faqSchemaLeaks = [];
for (const file of htmlFiles) {
  const html = await fs.readFile(file, 'utf8');
  for (const match of html.matchAll(/<script type="application\/ld\+json">(\{"@context":"https:\/\/schema\.org","@type":"FAQPage".*?\})<\/script>/g)) {
    let parsed;
    try {
    ...
    for (const question of parsed.mainEntity ?? []) {
      if (/^(參考資料|資料來源|延伸閱讀|相關工具)/.test(question.name ?? '')) {
        faqSchemaLeaks.push(`${toPosix(path.relative(distRoot, file))}: ${question.name}`);
      }
    }
  }
}
check('faq-schema-has-no-section-headings', faqSchemaLeaks.length === 0, faqSchemaLeaks.slice(0, 5));

// --- affiliate 一致性 ---------------------------------------------------
// 首頁曾經寫「商品與聯盟推薦在 AdSense 重新審查期間預設關閉」，同時文章頁
// 實際顯示蝦皮／酷澎商品卡。文案與實際行為必須由 src/config/affiliate.ts
// 這個單一開關決定，任何頁面都不得再自行 hardcode 狀態。
const homeHtml = await fs.readFile(path.join(distRoot, 'index.html'), 'utf8');
const homeSaysDisabled = /商品與聯盟推薦目前全站關閉/.test(homeHtml);
const someArticleShowsAffiliate = affiliateFiles.some(({ file }) => file.includes(`${path.sep}zh${path.sep}blog${path.sep}`));
check(
  'affiliate-copy-matches-behaviour',
  !(homeSaysDisabled && someArticleShowsAffiliate),
  `home says disabled=${homeSaysDisabled}, article cards rendered=${someArticleShowsAffiliate}`,
);
check(
  'affiliate-no-hardcoded-review-copy',
  !/審查期間預設關閉/.test(homeHtml),
  'the home page must derive its affiliate line from AFFILIATE_POLICY_LINE',
);

// --- GA4 affiliate tracking 未被破壞 ------------------------------------
// 重構 affiliate component 時最容易無聲弄丟的就是這些 data-* 掛勾。
if (someArticleShowsAffiliate) {
  const sample = affiliateFiles.find(({ file }) => file.includes(`${path.sep}zh${path.sep}blog${path.sep}`));
  for (const attribute of [
    'data-affiliate-recs',
    'data-affiliate-placement',
    'data-affiliate-surface',
    'data-affiliate-batch',
    'data-affiliate-module-sentinel',
    'data-affiliate-card',
    'data-affiliate-product-link',
    'data-affiliate-product-id',
    'data-affiliate-product-category',
    'data-affiliate-network',
    'data-affiliate-position',
  ]) {
    check(`affiliate-tracking-hook:${attribute}`, sample.html.includes(attribute), 'GA4 affiliate tracking hook must survive refactors');
  }
}
