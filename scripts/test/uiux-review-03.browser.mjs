...
const open = async (page, route) => {
  ...
  if (await rejectConsent.isVisible().catch(() => false)) await rejectConsent.click().catch(() => {});
  const consentWidget = page.locator('[data-consent-widget]');
  if (await consentWidget.isVisible().catch(() => false)) await consentWidget.evaluate((element) => { element.style.display = 'none'; });
