...
contract, FAQ-schema/page consistency, affiliate copy/behaviour consistency, and
the Merchant listings contract for `Product` structured data — see the checks at
the end of `scripts/content-quality-audit.mjs`.

Every `Product` JSON-LD node must be built by `buildProductJsonLd` in
`src/data/productSchema.ts`; `scripts/test/product-structured-data.test.mjs`
fails if a page hand-writes one. Google requires `image` for merchant listings
and validates `category` against its own product taxonomy, so the builder makes
at least one image mandatory at the type level and omits `category` rather than
guessing. Product cover art is generated by `npm run assets:products`. This is
how four paid landing pages shipped invalid merchant listings on 2026-09-18:
each page had its own hand-written Product node, none of them had `image`, and
all four used prose for `category`.
