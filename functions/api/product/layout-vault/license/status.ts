import { clearEntitlementCookies, productEntitlementConfig, revalidateProductEntitlement, revalidationRateLimited } from '../../../../_lib/entitlement.ts';
...
export async function onRequestGet({ request, env }: Context) {
  if (await revalidationRateLimited(request)) return new Response(JSON.stringify({ active: false, reason: 'rate_limited', nextCheckAt: Math.floor(Date.now() / 1000) + 60 }), { status: 429, headers });
...
export function onRequestPost() { return new Response(JSON.stringify({ active: false, reason: 'method_not_allowed' }), { status: 405, headers: { ...headers, allow: 'GET' } }); }
